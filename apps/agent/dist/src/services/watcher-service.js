...
import { generateChecksum, signedFetch, retryFetch } from '@tdc/sync-protocol';
...
export class WatcherService {
     ...
     */
    async buildChangeFromFile(path, event) {
        try {
    ...
    async buildProductChange(path, event) {
    ...
    async buildCategoryChange(path, event) {
        const id = this.extractIdFromPath(path);
        if (!id)
            return null;
        if (event === 'unlink') {
     ...
     */
    async pushChangeToCloud(change) {
        try {
            ...
            await retryFetch(() => signedFetch(`${this.cloudBaseUrl}/api/sync/push`, this.syncToken, batch), 3, 1000);
