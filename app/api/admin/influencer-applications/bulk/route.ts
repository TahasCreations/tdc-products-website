...
} from "@/lib/influencer-application-admin";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const results = await Promise.all(
      sanitizedIds.map(async (id) => {
        try {
          const result =
            action === "approve"
              ? await approveInfluencerApplication(id, processedBy)
              : await rejectInfluencerApplication(id, processedBy, reason);
          ...
          console.error(`Bulk ${action} influencer application error for ${id}:`, error);
          return {
            id,
    ...
    console.error("Bulk influencer application action error:", error);
    return NextResponse.json(
      {
        success: false,
